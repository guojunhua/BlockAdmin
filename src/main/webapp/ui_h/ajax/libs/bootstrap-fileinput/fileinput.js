...
(function (factory) {
    ...
    FileInput = function (element, options) {
        var self = this;
        self._initTemplateDefaults();//提取执行了

        if(options.initialPreviewConfig){
        	for(var i=0;i<options.initialPreviewConfig.length;i++){
        		var type = options.initialPreviewConfig[i]['type'];
        		if(type == null || type ==''){
        			for(var one in self.fileTypeSettings){
        				if(self.fileTypeSettings[one]('',options.initialPreviewConfig[i]['caption'])){
        					options.initialPreviewConfig[i]['type'] = one;
        					break;
        				}
    ...
    FileInput.prototype = {
        ...
        _parseError: function (operation, jqXHR, errorThrown, fileName) {
            ...
            var self = this, errMsg = $.trim(errorThrown + ''), textPre,
                text = jqXHR.responseJSON !== undefined && jqXHR.responseJSON.msg !== undefined ?
                    jqXHR.responseJSON.msg : jqXHR.responseText;
