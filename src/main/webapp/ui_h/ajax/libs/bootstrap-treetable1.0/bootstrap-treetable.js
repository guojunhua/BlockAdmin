/**
 * bootstrap-treetable
 * edit by jzhao 20200415
 * 1、$ipt.prop('checked', true).change(); 增加了.change();好监控事件
 * 2、增加 是否显示检索信息
 * v1.0.10-beta
 * @author swifly
...
(function($) {
        ...
        if (self.options.url) {
            $.ajax({
                ...
                success: function(data, textStatus, jqXHR) {
                	data = self.calculateObjectValue(self.options, self.options.responseHandler, [data], data);
            ...
            if(isAdd){
                ...
                if (_index > -1) {
                    ...
                    if(self.hasFixedColumn){
                        ...
                        $leftIpt.prop('checked', "").change();
                    }
                    $ipt.parent().parent().removeClass("treetable-selected");
                    $ipt.prop('checked', "").change();
                }
            }
        }
    };
    ...
    BootstrapTreeTable.prototype.calculateObjectValue = function (self, name, args, defaultValue) {
        var func = name;
