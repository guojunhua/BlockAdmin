...
var loadDialog = function($widget, name, url, width, height) {
	...
	// 弹出窗口（修改成 窗口内弹出，减少影响面，后面jq的弹出基本不用了）
// 	var dialog = parent.sy.modalDialog({
	var dialog = sy.modalDialog({
 		...
 		buttons : [ {
 			text : '&nbsp;&nbsp;保&nbsp;存&nbsp;&nbsp;',
 			handler : function() {
 				dialog.find('iframe').get(0).contentWindow.btnSaveCallback(dialog, $widget, $);
 			}
 		},
  		{
 			text : '&nbsp;&nbsp;取&nbsp;消&nbsp;&nbsp;',
	...
	$(win).keyup(function(event) {
		...
		if(event.ctrlKey && (event.keyCode == 10 || event.keyCode == 13)){
			// Ctrl + 回车
			win.btnSaveCallback(dialog, $widget, $);
		}
	});
};
...
//table确认选择
var confirmChoice = function($grid, parentField,nameField, minNum,maxNum){
	var rows = $grid.selectUniqueColumns();
	if (rows.length == 0) {
	...
	// 父页面的变量
	//${menuCode!}
	parent.$('#'+parentField+'_show').val($grid.selectColumns('nameField').join());
	parent.$('#'+parentField).val(rows.join());
