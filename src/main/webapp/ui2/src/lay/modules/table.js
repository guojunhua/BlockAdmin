...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  Class.prototype.render = function(){
    ...
    //请求数据 edit by jzhao，增加初次加载 显示 onload
  ...
  table.reload = function(id, options){
    ...
    //reload重置条件（保留order 以及 sort 2个排序参数）
    if(options!= null && options.initSort == null){//非排序，重新加载
    	var temp = {};
        for (var i in config.where)
        {
    	    if(i == 'order' || i == 'sort'){//属性名称
    	    	temp[i] = config.where[i];
    	    }
        }
        config.where = temp;
    }
