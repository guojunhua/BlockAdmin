/**
 ...
 @Author：贤心
 @License：MIT
 edit by jzhao（因为下拉复选所以覆盖了）2018-07-29
 https://github.com/yelog/layui-select-multiple
 
 edit by jzhao添加 只读控件（select 和 checkbox  以及radio），readonly 和disable的区别readonly只能看，disable能看到数据也拿不到 2018-07-29
 edit by jzhao添加 几个数据校验方法 remote length range match，以及隐藏或者不能显示提示用户错误，改成 只提示并提示用户哪个字段出现问题 2018-07-30
...
layui.define('layer', function(exports){
    ...
    var $ = layui.$
        ...
        ,SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled' ,READONLY = 'layui-readonly'

        ,Form = function(){
        this.config = {
        	//直接正则的除了 	required 其他的在都必须在required验证过了才才会验证（即非空才能验证）
            verify: {
                required: [
                    /[\S]+/
                    ,'必填项不能为空'
                ...
                //add by jzhao 2018-07-30 默认要把自己带上
                ,remote: function(value, item,param){
                	//param = path/to/server.php or path/to/server.php,id1,id2
                	...
                	params.forEach(function(value,index,array){
                		 if(index != 0){
                			 ...
                			 postParams[value] = inputValue;
                		 }                		 
                	　　});
                	
                	postParams[item.id] = $("#"+item.id).val();       
                ...
                ,length: function(value, item,param){//
                	//length[6~16]       //6-16个字符
                	//length[6]          //6个字符
                	//length[~6]         //小于6个字符
                	//length[6~]         //大于6个字符
                	...
                	if(temps.length == 1 && temps[0] != value.length){
                		return '输入字符长度非法,不为'+temps[0];
                	}else if(temps.length == 2){
                		
                		if(temps[0] == ''){//~6
                			if(value.length>=temps[1])
                				return '输入字符长度非法，不小于'+temps[1];
                		}else if(temps[1] == ''){//6~
                			if(value.length<=temps[0])
                				return '输入字符长度非法，不大于'+temps[0];
                		}else{//6~16
                			if(value.length<temps[0] || value.length>temps[1])
                				return '输入字符长度非法，不介于['+temps[0]+'~'+temps[1]+']';
                		}
                	}
                	
                }
                ,range: function(value, item,param){
                	if(value.length == 0)
                		return;
                	//range	必选，范围值	数值范围	
                	//range[0~99]      //0到99的整数
                	...
                	var temps = param.split("~");
                	 if(temps.length == 2){
                		...
                		if(temps[0] == ''){//~6
                			if(Number(value)>Number(temps[1]))
                				return '输入数字非法，未小于等于'+temps[1];
                		}else if(temps[1] == ''){//6~
                			if(Number(value)<Number(temps[0]))
                				return '输入数字非法，未大于等于'+temps[0];
                		}else{//6~16
                			if(Number(value)<Number(temps[0]) || Number(value)>Number(temps[1]))
                				return '输入数字非法，不介于['+temps[0]+'~'+temps[1]+']';
                		}
                	}
                	
                }
                ,match: function(value, item,param){//
                	if(value.length == 0)
                		return;
                	//match[password]  //与password字段的值匹配,如果有其他的匹配自己补充下 如：match[ id,condition];
                	var matchValue = $("#"+param).val();
            ...
            select: function(){
                ...
                selects.each(function(index, select){
                    var othis = $(this)
                        ...
                        ,readonly = ( othis.attr('readonly') === 'readonly')?true:false; //需要添加readonly样式
                        	;
                        var	noEvent = disabled||readonly; 		
                    ...
                    if (isMulti) {
                        var reElem = $(['<div class="' + (isMulti ? '' : 'layui-unselect ') + CLASS + (disabled||readonly ? ' layui-select-disabled' : '') + '">'
                            , '<div class="' + TITLE + '"><input type="text" class="layui-input" value="'+inputValue+'" placeholder="' + placeholder + ' " readonly><div class="layui-input multiSelect" >' + function(){
                                var aLists = [];
                        ...
                        othis.after(reElem);
                        events.call(this, reElem, noEvent, isMulti);
                    } else {
                        //替代元素
                        var reElem = $(['<div class="'+ (isSearch ? '' : 'layui-unselect ') + CLASS
                        
                            ,(disabled||readonly ? ' layui-select-disabled' : '') +'">'
                            ,'<div class="'+ TITLE +'">'
                            ,('<input type="text" placeholder="'+ placeholder +'" '
                               ...
                               //  +(readonly ? (' ' + READONLY) : ''))
                                + (disabled||readonly ? (' ' + DISABLED) : '') +'" readonly>') //禁用状态
                        ...
                        hasRender[0] && hasRender.remove(); //如果已经渲染，则Rerender
                        othis.after(reElem);
                        events.call(this, reElem, noEvent, isSearch);
                    }
            ...
            ,checkbox: function(){
                var CLASS = {
                    ...
                    reElem.on('click', function(){
                        ...
                        ,readonly = ( check.attr('readonly') === 'readonly')?true:false; //需要添加readonly样式
                        
                        if(check[0].disabled || readonly) return;
                ...
                checks.each(function(index, check){
                    ...
                    var hasRender = othis.next('.' + RE_CLASS[0])
                        ...
                        ,(disabled||readonly ? ' layui-checkbox-disbaled '+ DISABLED : '') //禁用状态
                        ,'"'
            ...
            ,radio: function(){
                var CLASS = 'layui-form-radio', ICON = ['&#xe643;', '&#xe63f;']
                    ...
                    reElem.on('click', function(){
                        ...
                        var readonly = ( radio.attr('readonly') === 'readonly')?true:false; //需要添加readonly样式
                        
                        if(radio[0].disabled || readonly) return;
                ...
                radios.each(function(index, radio){
                    ...
                    var reElem = $(['<div class="layui-unselect '+ CLASS
                        ...
                        ,(disabled|| readonly ? ' layui-radio-disbaled '+DISABLED : '') +'">' //禁用状态
    ...
    var submit = function(){
        ...
        layui.each(verifyElem, function(_, item){
            var othis = $(this)
                ,vers = othis.attr('lay-verify').split('|')   //xx , xx[1~2] , xx[a,b],xx[1-2]{a=3}疑问着支持条件控制本字段 {} 中的内容为条件
                ,verType = othis.attr('lay-verType') //提示方式
                ,value = othis.val();
            
             if(othis.attr('lay-verify') == ''|| othis.attr('lay-verify')==null)
            	 return ;
            	
            	
            	othis.removeClass(DANGER);
            layui.each(vers, function(_, thisVer){//xx , xx[1~2] , xx[a,b]
                var isPass //是否校验过
                    ,errorText = '' //错误提示文本
                   ;
                
                var condition = null;
                if(thisVer.indexOf('{') != -1){//存在条件，先检查条件是否成功，条件都是id>=<值 ，或者id!=3 id!=null的条件
                	condition = thisVer.substring( thisVer.indexOf('{')+1,thisVer.length-1);//a=3
                	thisVer = thisVer.substring(0, thisVer.indexOf('{'));//xx[1-2]
                	
                	//提取“!=” 这样的条件 （好像只要替换前面的ID=》id的值）
                	var conditionEq = null;
                	var conditionId = null;
                	var conditionVal = null;
                	if(condition.indexOf("!=") != -1){//2位的 !=
                		conditionEq = condition.substring(condition.indexOf("!"),condition.indexOf("!")+2);
                		
                		conditionVal = condition.substring(condition.indexOf("!")+2);
       
                		conditionId = condition.substring(0,condition.indexOf("!"));
                	}else{
                		//一个 ><= 中的一个,以及>= <=              		
                		var p = -1;
                		
                		if( (p=condition.indexOf(">=")) != -1){
                			
                			
                			conditionId = condition.substring(0,p);
                			conditionEq = condition.substring(p,p+2);
                			conditionVal = condition.substring(p+2);
                		}else if((p=condition.indexOf(">")) != -1){
                			conditionId = condition.substring(0,p);
                			conditionEq = condition.substring(p,p+1);
                			conditionVal = condition.substring(p+1);
                		}else if((p=condition.indexOf("<=")) != -1){
                			conditionId = condition.substring(0,p);
                			conditionEq = condition.substring(p,p+2);
                			conditionVal = condition.substring(p+2);
                	...
                	if(conditionEq){//
                		...
                		//执行条件校验(conditionId 不能为空判断？)
                    	var idValue = $.getValueBykey(conditionId);
                    	var checkEx = idValue+conditionEq+conditionVal;
                    	var b= eval(checkEx);
                    	if(b == false){//成立则后续需要校验，否则不校验了
                    		//isPass = true;
                    		//退出不需要校验了
                    		return;
                    	}
                ...
                var param = '';
                if(thisVer.indexOf('[') != -1 ){//带参数
                	
                	param = thisVer.substring(thisVer.indexOf('[')+1,thisVer.length-1);
                	thisVer = thisVer.substring(0,thisVer.indexOf('['));
                }else if( thisVer.indexOf('(') != -1){
                	param = thisVer.substring(thisVer.indexOf('(')+1,thisVer.length-1);
                	thisVer = thisVer.substring(0,thisVer.indexOf('('));
                }
                ...
                //匹配验证规则
                if(verify[thisVer]){
                     //isPass = isFn ? errorText = verify[thisVer](value, item,param) : !verify[thisVer][0].test(value);
                    
                    //edit by jzhao 0918 ,正则情况 值空则不验证
                    if(isFn){
                    	errorText = verify[thisVer](value, item,param);
                    	if(errorText == undefined || errorText == null)
                    		isPass = true;
                    	else 
                    		isPass = false;
                    }else{
                    	if(thisVer != 'required' && (value == null || value.length == 0) )//值为空正则不校验了
                    		isPass = true;
                    	else{
                    		if(value == null)
                    			value = '';
                    		isPass = verify[thisVer][0].test(value);
                    ...
                    if(!isPass){
                    	//console.log(othis[0].attr('title'));
                    	if(othis[0].type == 'hidden' && othis[0].title!=null && othis[0].title!='')
                    		errorText = othis[0].title+','+errorText;
