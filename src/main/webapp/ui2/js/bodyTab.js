...
layui.define(["element","jquery"],function(exports){
	var element = layui.element,
		...
		Tab = function(){
			this.tabConfig = {
				openTabNum : 15,  //最大可打开窗口数量 undefined||15
	...
	function treeTraverse(data,level){
		
		var ulHtml = '';
        for(var i=0;i<data.length;i++){
        	
        	var state = data[i].spread;
            ...
            if(data[i].children != undefined && data[i].children.length > 0){//根
                ulHtml += '<a>';
                //系统图标显示不了，暂时屏蔽
//              if(data[i].iconCls != undefined && data[i].iconCls != ''){
//                  if(data[i].iconCls.indexOf("icon-") != -1){
//                      ulHtml += '<i class="seraph '+data[i].iconCls+'" data-icon="'+data[i].iconCls+'"></i>';
//                  }else{
//                      ulHtml += '<i class="layui-icon" data-icon="'+data[i].iconCls+'">'+data[i].iconCls+'</i>';
//                  }
//              }
                ulHtml += $.showTag(data[i].iconCls)+'<cite>'+data[i].text+'</cite>';
                ulHtml += '<span class="layui-nav-more"></span>';
                ulHtml += '</a>';
                ulHtml += '<dl class="layui-nav-child">';
                
               var children = treeTraverse(data[i].children);
               ulHtml += children;
                
                ulHtml += "</dl>";
            }else{//叶子
            	//console.log(data[i].attributes.url);
            		...
            		//if(template == '' && (data[i].text == '系统日志' || data[i].text == '用户管理' || data[i].text == '角色管理')){
            		//	param = '?template=lay';
            		//}
            		
            		
            		var url = '..'+data[i].attributes.url+param;
            		//console.log(data[i].attributes.url+":"+data[i].attributes.url.indexOf('http'));
            		if(data[i].attributes.url.indexOf('http') == 0)
            			url = data[i].attributes.url+param;
            		
                    if(data[i].target == "_blank"){
                        ulHtml += '<a data-url="'+url+'" target="'+data[i].target+'">';
                    }else{
                        ulHtml += '<a data-url="'+url+'">';
                    }
...
//                    }
                ...
                ulHtml += $.showTag(data[i].iconCls) + '<cite>'+data[i].text+'</cite></a>';
            }
            ulHtml += '</li>';
        }
        return ulHtml;
