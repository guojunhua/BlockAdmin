...
var LayUIDataTable = (function () {
    ...
    /**
     * 转换数据表格。
     * @param callback 双击行的回调函数，该回调函数返回三个参数，分别为：当前点击行的索引值、当前点击单元格的值、当前行数据，触发事件
    ...
    function ConvertDataTable (callback) {
        ...
        $.each(trArr, function (index, trObj) {
            ...
            $(trObj).click(function (e) {//双击目前改成单击 dblclick =>click
            	//layui-table-hover
                ...
                callback(currentClickRowIndex, currentClickCellValue, returnData,'click');
            });
            $(trObj).dblclick(function (e) {//双击目前改成单击 dblclick =>click
            	//layui-table-hover
            	//console.log( '=='+$(this).attr('class'));
                ...
                callback(currentClickRowIndex, currentClickCellValue, returnData,'dblclick');
            });
