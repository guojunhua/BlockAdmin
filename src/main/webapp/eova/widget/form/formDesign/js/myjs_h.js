var view = null;
var objParameter= null;//进入的时候需要初始
//显示设置文本属性的模态框(还有个用处，已经移动到相应位置了，可以做后续事情) ，第一点击如果是隐藏的数据不现实，否则显示
function showAttribute(obj,id){//从objParameter种提取，如果无则从现在的规则提取
	//屏蔽拖拽
	...
	if(objParameter[id] == null){
		...
		//获取属性：id，默认值， 提示值，校验规则，  bb-verify="required=true;rangelength=[5,10]"，只读隐藏，
		//var defaultVal =1;
		//= inputObj.val();
	//		bb-verify           存在bb-verify字段
	//		placeholder         id字段
	//		defaultValue 当前值 id字段
		
		
		
		var id = obj.attr("id");
		var placeholder = obj.attr("placeholder");
		var verify = $(obj).parent().find("[bb-verify]");//
		if(verify[0] != null)
			verify = $(verify[0]).attr("bb-verify")
		else
			verify = "";
		...
		var ismust = false;
		if(verify){
			if(verify.indexOf("required=true") == 0){
				ismust = true;
				verify = verify.substring("required=true;".length);
				//if(verify.indexOf("|") == 0)
		...
		one = {defaultVal:defaultVal,placeholder:placeholder,verify:verify,readOnly:readOnly,ismust:ismust};
		objParameter[id] = one;
	}else{
		one = objParameter[id];
	}
	...
	//
	$("#obj_id").val(id);
	$("#obj_defaultValue").val(one.defaultVal);
	$("#obj_placeholder").val(one.placeholder);
	$("#obj_layVerify").val(one.verify);
...
$(function(){
	...
	$("#save-content").click(function(e){
		...
		if(id=="" || id==null){
			...
			var one = objParameter[id];
			one = {defaultVal:defaultValue,placeholder:placeholder,verify:layVerify,readOnly:readOnly,ismust:isMust};
			objParameter[id] = one;
