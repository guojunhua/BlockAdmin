...
%>
<%if(item.type == "下拉框"){%>
	<div><#combo id="${item.en}" name="${name}" code="${item.object_code}" field="${item.en}" value="${value!}" isNoN="${item.is_required!}" multiple="${item.is_multiple}" isReadonly="${readnoly!false}" /></div>
<%} else if(item.type == "查找框"){%>
    <div><#find id="${item.en}" name="${name}" code="${item.object_code}" field="${item.en}" value="${value!}" isNoN="${item.is_required!}" multiple="${item.is_multiple}" isReadonly="${readnoly!false}" /></div>
<%} else if(item.type == "时间框"){%>
	<%if(!isEmpty(value!)){value = strutil.formatDate(value, 'yyyy-MM-dd HH:mm:ss');}%>
    <div><#time id="${item.en}" name="${name}" value="${value!}" isNoN="${item.is_required!}" isReadonly="${readnoly!false}" options="format:'yyyy-MM-dd HH:mm:ss'" /></div>
<%} else if(item.type == "日期框"){%>
	<%if(!isEmpty(value!)){value = strutil.formatDate(value, 'yyyy-MM-dd');}%>
    <div><#time id="${item.en}" name="${name}" value="${value!}" isNoN="${item.is_required!}" isReadonly="${readnoly!false}" options="format:'yyyy-MM-dd'"/></div>
...
<%} else if(item.type == "自增框" || item.type == "文本框" || item.type == "数字框"){%>
    <div><#text id="${item.en}" name="${name}" value="${value!}" placeholder="${item.placeholder!}" isNoN="${item.is_required!}" validator="${item.validator!}" options="" isReadonly="${readnoly!false}" /></div>
<%} else if(item.type == "JSON框"){%>
	<div style="margin-left: 90px;"><#json id="${item.en}" name="${name}" value="${value!}" isNoN="${item.is_required!}" style="margin-top: 3px;width:99.9%;height:${item.height}px;" isReadonly="${readnoly!false}" /></div>
<%} else {// 默认为文本框%>
