...
<div class="input-group">
              <input id="${id!}_show" name="${name}_show" autocomplete="off" placeholder="${placeholderstr!}" ${disabled} class="form-control" >
		 ...
		 //如果是只读模式，则事件移除
 		 //tag移除事件
 	 	 ...
 	 	 $("#${id!}_show").click(function(){
 	 		...
 	 		$.modal.open("选择${item.cn!}", ctx + "single_grid/list/v_class?&mode=check&isQuery=false&choiceField=${id!}&nameField=name&choiceFieldValue="+$("#${id!}").val());
 	 	  });
