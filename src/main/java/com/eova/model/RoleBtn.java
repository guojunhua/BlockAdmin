...
//		return Db.use(xx.DS_EOVA).query("select bid from eova_role_btn where rid = ?", rid);
		...
		List<RoleBtn> roleBtns = this.queryByCache("select * from eova_role_btn where rid = ?", rid);
...
//		return Db.use(xx.DS_EOVA).query("select bid from eova_role_btn where rid in "+DbUtil.joinIds(rids));
		...
		List<RoleBtn> roleBtns = this.queryByCache("select bid from eova_role_btn where rid in "+DbUtil.joinIds(rids));
		List<Integer> roleBtnIds = roleBtns.stream().map(b -> {
			   return b.getInt("bid");
			}).collect(Collectors.toList());
		return roleBtnIds;
